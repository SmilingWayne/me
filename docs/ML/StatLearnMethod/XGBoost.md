...
## XGBoost (eXtreme Gradient Boosting) 算法
...
*   **训练复杂度**：
    ...
    *   对于**近似算法**，复杂度则变为 $O(M \cdot d \cdot \text{num}_{\text{candidates}} \cdot \log N)$，其中 `num_candidates` 是候选分裂点的数量，远小于 $N$。这使得 XGBoost 在处理大规模数据时效率极高。
