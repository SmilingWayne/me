...
## current hot topics in Transportation in China.
...
- 决策：决定when,where,how long each bus should be charged
    ...
    - $v_{l,j,s}$ 在离开第j充电站时，线路l的公交的SOC是s，那么就是1，否则为0（0-1 bin）
...
- 参数含义
    - D_{l,j} 线路l的公交，从j站驶向j+1站点的电量损耗
    - H： 从 SOC = 0的情况到完全充电（100%）的充电总时长
    - $\tau$ 
    - s 是充电的interval数，比如从0开始充 s = 2, 那么就相当于冲2个interval， $\phi(\tau)$ 是时间和充电状态之间的反向映射
