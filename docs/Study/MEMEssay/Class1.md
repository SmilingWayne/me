...
> CIMO框架: 情境 (Context); 干预 (Intervention):解决方案;机制 (Mechanisms):设计假设，干预产生的作用形式，在什么背景下，这些机制才能发挥作用。 结果(Outcomes)


!!! example "JIT and CIMO框架"
    ...
    - 库存成本是结果(O)
    - 汽车组装线是背景(C)
    - 其中的机制(M)是JIT的组成，比如用看板引导物流

- 设计假设与具体方案的关系
    - 设计方案是对设计假设的具体化
    - 设计假设与设计方案的一对一、一对多、多对多的关系 
    - 设计假设的确定性与解决方案的多样性
...
- 搜索解决方案
    - 顺序性启发式搜索 (Gigerenzer, Reb, & Luan, 2022)。 ü 搜索线索:
        - 是否有卓越能力
        - 是否是让人敬佩的
        - 他能否提升进入团队的平均水平。
    - 搜索规则:搜索线索的排序。
        - 如从卓越能力开始，然后是是否让人敬佩，最后是是否能提升团队平均水平。
    - 停止规则:某一线索提示是结束。
    - 决策规则:依据结束所需要的行为，如聘用、不聘用。


!!! example "最小化假阳性 vs 最小化措施"
    
    最小化假阳性 vs 最小化错失(聘用，但能力并未都满足)。
    
    - 最小化阳性中考虑以满足卓越能力的人，作为后续两个线索的候选人
    - 最小化错失是即便是不满足卓越能力这一项，仍然作为后续两个线索的候选人，并且在
    
    > 满足任一线索条件都可以雇佣。

## 1221 解决方案评估 

评估:对解决方案的属性和某些特征的系统评价。

评估的目的

1. 证实解决方案的有效性:解决方案与结果之间存在不可预测因素 
2. 进行理论验证
3. 为解决方案的修改和调整提供依据
4. 为设计实施方案提供基础
5. 验证责任性、传播和启发
