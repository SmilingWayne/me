...
!!! info "什么是LoRA（Low Rank Adaptation）"
    是大型语言模型中使用的一种方法，用于提高其下游任务的性能，特别是那些需要生成连贯且流畅的文本的任务。 它是由基于 Transformer 的语言模型 BERT（来自 Transformers 的双向编码器表示）的作者在他们的论文“BERT：用于语言理解的深度双向 Transformers 的预训练”中介绍的。
