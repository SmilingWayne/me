...
- 运输问题基变量的个数是$m + n - 1$，前$m$行之和等于后$n$行之和，所以$r(A) < m + n$，这意味着任何$m + n$个决策变量的系数列向量都线性相关，$rank(A) = m + n - 1$。如果可以找到$m + n - 1$个线性无关的系数列向量，就可以得到一个相应的基本可行解。
- 运输问题一定存在最优解
- 如果运输问题的供给量$a_i(i = 1,2,..,m)$和需求量都是整数，那么该问题的基本可行解均为整数解，这个性质表明，在运输问题中，可以不必考虑决策变量$x_{ij}$是否有整数约束，可用标准的线性规划方法求解。
